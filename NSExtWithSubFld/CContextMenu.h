...
public:
								...
								UINT	uFlags)
	{
		...
		if(	NULL == m_pidl ) 
		{
			...
			::InsertMenu(hMenu,	iIndexMenu++, MF_STRING	| MF_BYPOSITION, idCmdFirst	+ IDM_CREATE_FOLDER, _TEXT("New Shoot"));
		...
		{
			::InsertMenu(hMenu,	iIndexMenu++, MF_STRING	| MF_BYPOSITION, idCmdFirst	+ IDM_DELETE, _TEXT("&Delete Shoot"));
			::InsertMenu(hMenu, iIndexMenu++, MF_SEPARATOR | MF_STRING | MF_BYPOSITION, 0, _T(""));
			::InsertMenu(hMenu,	iIndexMenu++, MF_STRING	| MF_BYPOSITION, idCmdFirst	+ IDM_PROPERTIES, _TEXT("&Properties"));
		}
