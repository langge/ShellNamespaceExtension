// NSFShellView.cpp : Implementation of CNSFShellView
#include "stdafx.h"
...
TCHAR  g_szMenuItemCaptions_Chinese[][20]=
			{ _TEXT("»èÁ¦(&D)"),_TEXT("¼Ó¼º(&R)"),_TEXT("´Ù½ÃÀÐ±â(&e)"),_TEXT("About NSF(&A)")};
...
LRESULT CNSFShellView::_ViewActivate(UINT uState)
{
    ...
    if( uState!=SVUIA_DEACTIVATE ) 
    {
		...
		{	    		
			...
			{
				if(TRUE == LocaleIsChinese())
				{
					::InsertMenu ( mii.hSubMenu, 0xFFFFFFFF, MF_BYPOSITION,
							//ID_HELP_HELPTOPICS, TEXT("NSF°ïÖú(&N)") );
					ID_HELP_HELPTOPICS,g_szMenuItemCaptions_Chinese[MI_HELPTOPICS_INDEX]);
				}
				else
				{
					::InsertMenu ( mii.hSubMenu, 0xFFFFFFFF, MF_BYPOSITION,
							//ID_HELP_HELPTOPICS, TEXT("&NSF Help") );
			...
			{
				//ATLASSERT(_MergeFileMenu(mii.hSubMenu)==TRUE);
				if(TRUE == LocaleIsChinese())
				{
					...
					if(SVUIA_ACTIVATE_FOCUS == uState)
					{
						...
						::InsertMenu(mii.hSubMenu,0,MF_BYPOSITION,
							 //ID_EDIT_PROPERTIES,TEXT("ÊôÐÔ(&R)") );
						ID_EDIT_PROPERTIES,g_szMenuItemCaptions_Chinese[MI_PROPERTIES_INDEX]);

						::InsertMenu(mii.hSubMenu,0,MF_BYPOSITION,
								 //ID_EDIT_DELETE,TEXT("É¾³ý(&D)") );
						ID_EDIT_DELETE,g_szMenuItemCaptions_Chinese[MI_DELETE_INDEX]);
					}
				...
				else
				{
					if(SVUIA_ACTIVATE_FOCUS == uState)
					{
						::InsertMenu(mii.hSubMenu,0,MF_BYPOSITION|MF_SEPARATOR,
							 0, NULL );
						::InsertMenu(mii.hSubMenu,0,MF_BYPOSITION,
							 //ID_EDIT_PROPERTIES,TEXT("P&roperties") );
...
LRESULT CNSFShellView::OnItemActivated(UINT CtlID, LPNMHDR lpnmh, BOOL& bHandled)
{
   // ATLTRACE(_T("CNSFShellView::OnItemActivated\n"));
    ...
    if(ListView_GetItem(m_hwndList, &lvItem))
    {
        ...
        {
            //MessageBox(NULL,"dbclick!","NSF",MB_OK);
