...
TCHAR  g_szMenuItemCaptions_English[][20]=
			{ _TEXT("&Delete"),_TEXT("P&roperties"),_TEXT("R&efresh"),_TEXT("&About NSF")};
...
BOOL CNSFShellView::_InitListView()
{
	...
	TCHAR   szString[3][20] = {_TEXT("Name"), _TEXT("Type")};  //empty the list

	int     iWidth[2] = { 240 , 120  };
...
LRESULT CNSFShellView::OnContextMenu(UINT uMsg, WPARAM wParam, LPARAM lParam, BOOL& bHandled)
{
    ATLTRACE(_T("CNSFShellView::OnContextMenu\n"));
	LPITEMIDLIST pidlSelected=NULL;
	LPITEMIDLIST *apidls;
	
	apidls = (LPITEMIDLIST*)_Module.m_Allocator.Alloc(1 * sizeof(LPITEMIDLIST));  
	if(apidls == NULL)
	...
	m_pFolder->GetUIObjectOf(m_hWnd, 1, (LPCITEMIDLIST*)apidls, 
	...
	if(pContextMenu)
	{
			               ...
			               ( hMenu,0,MENU_OFFSET, MENU_MAX,CMF_NORMAL) ) )
		{
			...
			if( iSelCmdItem > 0)
			{
				...
				cmi.hwnd = m_hWnd;           
				cmi.lpVerb = (LPCSTR)MAKEINTRESOURCE(iSelCmdItem - MENU_OFFSET);       
...
LRESULT CNSFShellView::OnNewFolder(WORD /*wNotifyCode*/, WORD /*wID*/, HWND /*hWndCtl*/, BOOL& /*bHandled*/)
{
	TCHAR szTempNewName[MAX_PATH]=_T("");
	
	//1.Add dir item "New Folder" to current folder's section in configuration file
	...
	tmpPidl1=m_PidlMgr.Concatenate(_Module.m_pidlNSFROOT,m_pFolder->m_pidlPath);
	tmpPidl2=m_PidlMgr.Concatenate(tmpPidl1,(LPCITEMIDLIST)lvItem.lParam);
    ::SHChangeNotify(SHCNE_MKDIR, SHCNF_IDLIST | SHCNF_FLUSH, tmpPidl2, NULL);
	m_PidlMgr.Delete(tmpPidl1);
	m_PidlMgr.Delete(tmpPidl2);
 
    //5.Put ListView item in edit mode...
    ::SetFocus(m_hwndList);
...
LRESULT CNSFShellView::OnLabelEditBegin(UINT /*CtlID*/, LPNMHDR lpnmh, BOOL& /*bHandled*/)
{
   ATLTRACE(_T("CNSFShellView::OnLabelEditBegin\n"));
...
LRESULT CNSFShellView::OnLabelEditEnd(UINT /*CtlID*/, LPNMHDR lpnmh, BOOL& /*bHandled*/)
{
	ATLTRACE(_T("CNSFShellView::OnLabelEditEnd\n"));
	NMLVDISPINFO* lpdi = (NMLVDISPINFO*) lpnmh;

	if( ( lpdi->item.pszText==NULL ) || (_tcscmp(lpdi->item.pszText,_T("New Folder")) == 0) )
		return FALSE; // User cancelled editing

	if( _tcspbrk(lpdi->item.pszText, _T("<>/?:\"\\")) != NULL )	    
	{
		MessageBox(NULL,_T("File name can't include following characters: \\/:*?\"<>|"), \
		   _T("Error"),MB_OK);
		::SetFocus(m_hwndList);
	...
	{
		MessageBox(NULL,_T("Rename error: Specified foldername is equal to a folder existed! Please specify another name."), \
		   _T("Error"),MB_OK);
	    ::SetFocus(m_hwndList);
	    ListView_EditLabel(m_hwndList,lpdi->item.iItem);
	    return FALSE;
    ...
    if( FAILED(Hr) || (pidlNewItem==NULL) ) {
        ::MessageBox(NULL,_T("IShellFolder::SetNameOf failed."),_T("Error"),MB_OK);
        return FALSE;
	}
    ...
    //Refresh Other Shell View to show new name
    RefreshShellViewWndsExcept(m_hWnd);
